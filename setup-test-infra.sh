...
ALB_NAME="ecs-alb-$(date +%s)" # aleatorio con fecha y hora
...
# Procesar subnet_ids para convertirlo en formato de lista para Terraform
IFS=',' read -r -a subnet_array <<< "$SUBNET_IDS"
subnet_terraform_list=""
for subnet in "${subnet_array[@]}"; do
  if [ -n "$subnet_terraform_list" ]; then
    subnet_terraform_list+=", "
  fi
  subnet_terraform_list+="\"$subnet\""
done

cat <<EOF > terraform.tfvars
vpc_id = "${VPC_ID}"
subnet_ids = [${subnet_terraform_list}]
alb_name = "${ALB_NAME}"
EOF
